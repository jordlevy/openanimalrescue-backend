...
const VOLUNTEER_SIGNUP_TABLE_NAME =
  process.env.VOLUNTEER_SIGNUP_TABLE_NAME || "";
...
export const handler = async (event: any) => {
  try {
    ...
    const { eventId, volunteerId, assignedRole, action } = JSON.parse(
      event.body
    );
    ...
    if (!eventId || !volunteerId || !assignedRole || !action) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error:
            "Event ID, Volunteer ID, Assigned Role, and Action are required.",
        }),
      };
    }
    ...
    const volunteerResponse = await dynamoDb.send(
      new GetItemCommand(volunteerParams)
    );
    ...
    const volunteerUpdate: TransactWriteItem = {
      Update: {
        ...
        ExpressionAttributeValues: {
          ...
          ":reviewedAt": { N: generateEpoch(Date.now()).toString() },
    ...
    if (countAdjustment !== 0) {
      const countField =
        assignedRole === "exec" ? "approvedExecCount" : "approvedStandardCount";
